 ...
 * @property {BufferEncoding} [encoding=null] If specified, then buffers will
 *   be decoded to strings using the specified encoding.
 * @property {boolean} [objectMode=false] Whether this stream should behave as
 *   a stream of objects. Meaning that stream.read(n) returns a single value
 ...
 *   stream.write().
 * @property {boolean} [emitClose=true] Whether or not the stream should emit
 *   'close' after it has been destroyed.
 ...
 * @property {AbortSignal|undefined} [signal=undefined]
 ...
 *   side of the stream. Has no effect if objectMode is true.
 * @property {boolean} [bigReadError=false] By default, if you try to
...
/**
 * The stream was truncated earlier than expected by the reader.
 */
export class TruncationError extends Error {
   ...
   */
  constructor(size, offset, got = undefined) {
    let str = `Stream finished before ${size} bytes were available at offset ${offset}`;
    if (got !== undefined) {
      str += `, but only got ${got}`;
    }
    super(str);
    this.size = size;
    this.offset = offset;
...
class NoFilter extends Transform {
  ...
  constructor(input, inputEncoding, options = {}) {
    ...
    const bigReadError = Boolean(options.bigReadError);
    delete options.bigReadError;
    super(options);

    this.#readError = readError;
    this.#bigReadError = bigReadError;
  ...
  read(size) {
    ...
    if (buf != null) {
      ...
      if (this.#readError && (buf.length < size)) {
        throw new TruncationError(size, this.#offset, buf.length);
      }
      this.#offset += buf.length;
    } else if (this.#readError) {
      throw new TruncationError(size, this.#offset);
    }
    return buf;
  }

  /**
   * Wait for the full number of bytes asked for, no matter how long it takes.
   ...
   * @returns {Promise<void>} A promise for the data read.
   */
  waitFor(size) {
    let onReadable = null;
    let onFinish = null;
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.writableFinished) {
        ...
        reject(new TruncationError(size, this.#offset));
        return;
      }

      if (this.#bigReadError) {
        if (size > this.readableHighWaterMark) {
          reject(new Error(`Reading too many bytes: ${size} > ${this.readableHighWaterMark}`));
          return;
        }
      } else if (size > this.readableHighWaterMark) {
  ...
  slice(start, end) {
    ...
    switch (bufs.length) {
      ...
      case 1: return bufs[0].subarray(start, end);
      default: {
